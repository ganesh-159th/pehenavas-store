...
const SignUp = () => {
  ...
  const validate = () => {
    ...
    if (!agreeTerms)       newErrors.agreeTerms = 'You must agree to the terms and conditions to create an account.';
    setErrors(newErrors);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#faf6f0] to-amber-50 flex items-center justify-center px-4 py-12 overflow-hidden">
      <div className={`w-full max-w-md transition-all duration-1000 ease-out transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-6 text-sm">Join the royal heritage</p>
        </div>

        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-amber-100">
          <div className="h-1 bg-gradient-to-r from-rose-950 via-amber-500 to-rose-950"></div>
          <div className="p-8">
            <h2 className="text-xl font-bold text-rose-950 mb-6">Create Account</h2>
            <form className="space-y-5" onSubmit={handleSignUp}>
              <div>
                <label htmlFor="name" className="block text-sm font-semibold text-gray-700 mb-2">Full Name</label>
                <input id="name" name="name" type="text" autoComplete="name"
                  className={`w-full px-4 py-2.5 border-2 rounded-lg focus:ring-2 focus:ring-amber-400 focus:border-amber-500 transition-all outline-none ${errors.name ? 'border-rose-500 bg-rose-50' : 'border-gray-200'}`}
                  value={name} onChange={(e) => { setName(e.target.value); if (errors.name) setErrors({ ...errors, name: null }); }} placeholder="Your Name" />
                {errors.name && (
                  <div className="flex items-start gap-2 mt-2 p-2.5 bg-rose-50 border border-rose-200 rounded-md shadow-sm">
                    <AlertCircle className="w-4 h-4 text-rose-600 flex-shrink-0 mt-0.5" />
                    <p className="text-rose-800 text-xs font-medium leading-tight">{errors.name}</p>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input id="confirmPassword" name="confirmPassword" type={showConfirmPassword ? "text" : "password"} autoComplete="new-password"
                    ...
                    value={confirmPassword} onChange={(e) => { setConfirmPassword(e.target.value); if (errors.confirmPassword) setErrors({ ...errors, confirmPassword: null }); }} placeholder="••••••••" />
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-amber-500 focus:outline-none transition-colors">
              ...
              <div className="flex flex-col">
                <div className="flex items-center">
                  <input id="terms" type="checkbox" checked={agreeTerms}
          ...
          <div className="px-8 py-4 bg-gradient-to-r from-rose-50 to-amber-50 border-t border-amber-100">
            <p className="text-center text-xs text-gray-600">
              ...
              <Link to="/signin" className="font-semibold text-amber-600 hover:text-rose-950 transition-colors">Sign In</Link>
            </p>
          </div>
        </div>
      </div>
